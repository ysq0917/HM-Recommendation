...
st.sidebar.title('H&M  Recommendations')
...
def customer_article_recommend(customer,n_recs):
    ...
    # Add 
    # make sure customer is in the df_customer
    if customer not in df_customer.index:
        return 'Customer not found. Please enter a valid customer ID.'
    ...
    # not_bought = not_bought[~not_bought['article_id'].isin(have_bought)]
...
def article_recommend(article_input, n_recs2):
    ...
    # if article.empty:
    #     return 'Article not found. Please enter a valid article ID.'
...
def print_image_cf(results_cf, n_recs):
    ...
    for index, data in enumerate(article_id_cf):
        ...
        desc = ' '.join(desc_list)
        img = mpimg.imread(f'Data/images/0{str(data)[:2]}/0{int(data)}.jpg')
        ax[i].imshow(img)
        ax[i].set_xticks([], [])
        ax[i].set_yticks([], [])
        ax[i].grid(False)
        ax[i].set_xlabel(desc, fontsize=70)
        i += 1
    return plt.show()
...
if page == 'Existing Customer':
    ...
    rec_button = st.button("Get some recommendations...", key="existing_customer_button")
    if rec_button:
    ...
    book_button = st.button("Get some recommendations...", key="similar_article_button")
    if book_button:
